...
using static Dpr.AutomationFramework.Dpr.AutomationFramework.Savings.AvailableApplicants.ApplicantTypeList;

namespace Dpr.AutomationFramework.Dpr.AutomationFramework.Savings.AvailableApplicants
{
        ...
        public List<List<string>> ApplicantTypeRepo(ApplicantTypes applicantType, List<List<string>> updatedValues)
        {
            switch (applicantType)
            {
                ...
                case ApplicantTypes.Foreign:
                    updatedValues.Add(new List<string> { "EAP04", "countryOfBirth", "Italy" });
                    updatedValues.Add(new List<string> { "EAP04", "placeOfBirth", "Turin" });

                    break;

                // Changning DOB of applicant to 16 years ago.
                case ApplicantTypes.Child:
                    DateTime birthday = DateTime.Now.AddYears(-16).Date;
                    string year = birthday.Year.ToString();
                    string month = birthday.Month.ToString();
                    if (month.Length == 1) month = "0" + month;
                    string day = birthday.Day.ToString();
                    if (day.Length == 1) day = "0" + day;
                    updatedValues.Add(new List<string> { "EAP04", "dateOfBirth", day+"/"+month+"/"+year });
